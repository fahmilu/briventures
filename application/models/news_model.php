...
// fungsi untuk bagian admin
	...
	public function getwhitepaper(){
		$q = $this->db->get_where('site_whitepaper', array('id' => 1));
		return $q->row();
	}	

	function get_archived(){
		return $this->db->query('SELECT YEAR(site_news.date) AS YEAR, 
                COUNT(*) AS TOTAL 
         FROM site_news GROUP BY YEAR ORDER BY YEAR DESC');
	}
	...
	function insert(){
		...
		$configs['upload_path'] = './uploads/news/';
		...
		if(!empty($_FILES['userfile']['tmp_name']) && $_FILES['userfile']['tmp_name'] != 'none'){		
			...
			$configs['file_name']    = "news_".date("Y-m-d_His", time());
			$this->upload->initialize($configs);
		
			if ($this->upload->do_upload('userfile')){
				
				$data = $this->upload->data();
				$img_path='./uploads/news/'.$data['file_name'];
				$new_thumb='./uploads/news/thumbs/'.$data['raw_name'].$data['file_ext'];
	...
	function update(){
		...
		if(!empty($_FILES['userfile']['tmp_name']) && $_FILES['userfile']['tmp_name'] != 'none'){		
			...
			if ($this->upload->do_upload('userfile')){
				...
				$this->session->set_flashdata('error', $this->upload->display_errors());
				redirect('webadmin/news/edit/'.$id);
			}
		}
	...
	function updatewhitepaper(){
		...
		$this->db->where('id', 1);
		$update = $this->db->update('site_whitepaper', $updatedb);
